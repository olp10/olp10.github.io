...
window.onload = function init() {
    ...
    lightSpecular = vec4( light, light, light, 1.0 );
    ...
    // Add event listener that lowers lightSpecular = vec4( 0.2, 0.2, 0.2, 1.0 ) by 0.1 when left arrow button pressed
    window.addEventListener("keydown", function(e){
        if (e.key === 'ArrowRight' && light > 0.0) {
            light -= 0.05;
            lightSpecular = vec4( light, light, light, 1.0 );
            specularProduct = mult(lightSpecular, materialSpecular);
            gl.uniform4fv( gl.getUniformLocation(program, "specularProduct"), flatten(specularProduct) );
        ...
        if (e.key === 'ArrowDown' && pulsationSpeed > 0.01) {
            pulsationSpeed -= 0.01;
        }
    })
...
function render() {
    scaleValue += pulsationSpeed;
    var pulse = 0.5 * Math.sin(scaleValue) + 1.0;
    ...
    modelViewMatrix = mult( modelViewMatrix, scalem(pulse, pulse, pulse) );
